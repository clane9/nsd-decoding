"""v4: Residual MLP on PCA-projected (whitened) input."""
...
def load_split_tensors(ds, mask, pca, n_components):
    """Load a HF dataset split, apply mask, z-normalize, PCA project + whiten."""
    ...
    # PCA project + whiten
    components = pca["components"][:n_components]  # (n_components, 18577)
    pca_mean = pca["mean"]  # (18577,)
    scale = pca["scale"][:n_components]  # (n_components,)
...
def main(args):
    ...
    for split in dataset_dict:
        act, tgt = load_split_tensors(dataset_dict[split], mask, pca, args.n_components)
